...
A tuning of the `lines()` function:
...
A function that shifts the values of the vector `x` by `n` positions to the right,
keeping the length of the vector constant and filling the first `n` positions of the
...
Here we want to apply the constraint by accounting for the fact that age increases
between samples collection. This means that the above Step 2a needs to be adjusted
accordingly. More specifically, the monotonically increasing binomial GAM should not be
applied for each age value, but for each cohort, a cohort being defined by a given age
in the first collection time. The code below is the above function
`age_profile_constrained_cohort()` that has been adapted accordingly. For easier
comparison, we've left the old code:

```{r}
age_profile_constrained_cohort <- function(data, age_values = seq(0, 15, le = 512),
                                           ci = .95, n = 100) {
...
#START#################################################################################
# Code to translate age and collection time into cohort and vice versa:
...
Computing the new sero-prevalence profile (taking another 1'20"):

```{r eval = FALSE}
constrained_age_profiles_cohort <- age_profile_constrained_cohort(hfmd)
```

```{r include = FALSE}
if (file_exists("constrained_age_profiles_cohort.rds")) {
  constrained_age_profiles_cohort <- readRDS2("constrained_age_profiles_cohort.rds")
} else {
  constrained_age_profiles_cohort <- age_profile_constrained_cohort(hfmd)
  saveRDS2(constrained_age_profiles_cohort, "constrained_age_profiles_cohort.rds")
}
```
...
```{r}
plot_profiles(constrained_age_profiles_cohort)
```

### With extrapolation

Here is an alternative version of the above `age_profile_constrained_cohort()` function
that performs extrapolation over the whole age range for each time point. Same as
before, we highlight the code changes in step 2b:
...
Let's compute the values (another 1'20"):
...
```{r include = FALSE}
if (file_exists("constrained_age_profiles_cohort2.rds")) {
  constrained_age_profiles_cohort2 <- readRDS2("constrained_age_profiles_cohort2.rds")
} else {
  constrained_age_profiles_cohort2 <- age_profile_constrained_cohort2(hfmd)
  saveRDS2(constrained_age_profiles_cohort2, "constrained_age_profiles_cohort2.rds")
}
```

And here is what the plot now looks like:

```{r}
plot_profiles(constrained_age_profiles_cohort2)
```


## Attack rates

Computing the attack rates:
...
A function to plot the frame of a plot:

```{r}
plot_frame <- function() {
  plot(NA, xlim = c(0, 14), ylim = c(0, .3),
       xlab = "age (years)", ylab = "attack rate")
  abline(h = 0)
}
```

Plotting the attack rates using age in the first, second and third time points for the
three lines respectively:

```{r}
cols <- 2:4
plot_frame()
walk2(attack_rates, cols, ~ with(.x, lines2(age.x, attack, col = .y)))
```

Plotting the attack rates using age in the first time point for the three lines:

```{r}
plot_frame()
walk2(attack_rates, cols, ~ with(.x, lines2(cohort, attack, col = .y)))
```

## Expected incidences

Here we want to compute the expected incidence as

$$
(1 - p)\times \rho \times N
$$
where $p$ is the sero-prevalence at at time $t$ for age $a$, $\rho$ is the attack rate
for age $a$ between time points (*i.e.* collection) $t$ and $t+1$, and $N$ is the size
of the population of age $a$. Let's first load the 2019 Vietnamese population census
and extracting data for the province of HCMC:

```{r eval = FALSE}
census2019 <- "/Users/MarcChoisy/Library/CloudStorage/OneDrive-OxfordUniversityClinicalResearchUnit/data/census VNM 2019/census2019.rds" |>
  readRDS() |> 
  filter(province == "Thành phố Hồ Chí Minh")
```

```{r include = FALSE}
if (file_exists("census2019.rds")) {
  census2019 <- readRDS2("census2019.rds")
} else {
  census2019 <- "/Users/MarcChoisy/Library/CloudStorage/OneDrive-OxfordUniversityClinicalResearchUnit/data/census VNM 2019/census2019.rds" |>
    readRDS() |> 
    filter(province == "Thành phố Hồ Chí Minh")
  saveRDS2(census2019, "census2019.rds")
}
```

Computing the age pyramid for the province of HCMC:
...
Modelling the pyramid:

```{r}
mod <- lm(n ~ age, age_structure)
```

Computing the expected incidences:
...
Vizualizing the expected incidences:

```{r}
plot(NA, xlim = c(0, 15), ylim = c(0, 20000),
     xlab = "age (years)", ylab = "number of infections")
abline(h = 0)
walk2(incidences, cols, ~ with(.x, lines2(age.x, incidence, col = .y)))
```

Now that we have the expected age profiles of incidence between the three successive
time points, we want to compare it with the age profiles of hospitalization between the
same time points. By taking the ratio of the first of the second one, we can have an
idea of the probability of severity as a function of age.