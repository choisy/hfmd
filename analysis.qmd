...
step2b <- function(col_id = 6) {
  dat <- filter(simulations_step_2a, collection == col_id)
  dat$fit <- predict(gam(fit ~ s(age), betar, dat), type = "response")
  dat$upr <- predict(gam(upr ~ s(age), betar, dat), type = "response")
  dat$lwr <- predict(gam(lwr ~ s(age), betar, dat), type = "response")
  dat
}
```
